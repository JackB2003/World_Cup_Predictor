...
function TitleRaceRow({ t, max, rank }: { t: Team; max: number; rank: number }) {
  return (
    <div className="flex items-center gap-3">
      <span className="num text-(--text-dim) w-[22px] text-lg text-right">{rank}</span>
      ...
      <span className="num w-14 text-right text-xl">{t.titleProb}<span className="text-(--text-dim) text-xs">%</span></span>
      <Delta trend={t.trend} />
    </div>
  );
}
...
function PowerRow({ t, rank }: { t: Team; rank: number }) {
  return (
    <div className="lift flex items-center gap-3 p-3 rounded-xl border border-(--line)">
  ...
  return (
    <div className="fade-in grid gap-[18px]" style={{ gridTemplateColumns: "1.5fr 1fr" }}>
      <div className="card p-5">
        ...
        <div className="text-(--text-dim) text-[11.5px] mt-4 flex items-center gap-1.5">
      ...
      <div className="grid gap-[18px]">
        <div className="card p-5">
          ...
          <div className="flex flex-col gap-2.5">
            {top4.map((t, i) => (
              <div key={t.code} className="lift flex items-center gap-3 p-2.5 rounded-xl bg-(--surface-2)">
                ...
                <div className="text-right">
                  <div className="num text-lg text-(--accent)">{t.top4}%</div>
          ...
          <div className="flex flex-col gap-2">
            {data.modelWeights.map((w) => (
              <div key={w.factor} className="flex items-center gap-2 text-[13px]">
                ...
                <span className="text-(--text-mid) flex-1">{w.factor}</span>
