    ...
    <div className="card p-5">
      ...
      <div className="flex items-center gap-3 mb-4 p-3 rounded-[11px] bg-[rgba(var(--accent-rgb),0.08)] border border-[rgba(var(--accent-rgb),0.2)]">
        ...
        <div className="text-[13px] text-(--text-mid)">
          <span className="text-(--text) font-bold">{top4OnTrack} of 4</span> top-4 picks still on track.
      ...
      <div className="flex flex-col gap-2 mb-4">
        {top4.map((p) => {
          ...
          return (
            <div key={p.pos} className="flex items-center gap-3 p-2.5 rounded-[11px] bg-(--surface-2)">
              <span className="num text-(--text-dim) w-[18px] text-lg">{p.pos}</span>
              <Crest team={t} size={28} />
              <div className="flex-1 min-w-0">
                <div className="font-bold text-[13.5px]">{t?.name ?? p.team}</div>
                <div className="text-(--text-dim) text-[11px]">{p.note}</div>
              </div>
              <div className="text-right shrink-0">
                <div className="num text-[15px] text-(--accent)">{top4Prob}%</div>
                <div className="text-[10px] text-(--text-dim)">top-4 prob</div>
              </div>
      ...
      <div className="flex items-center gap-3 p-2.5 rounded-[11px] bg-(--surface-2)">
        <Crest team={teamMap[topScorer.team]} size={28} />
        <div className="flex-1 min-w-0">
          <div className="font-bold text-[13.5px]">{topScorer.player}</div>
          <div className="text-(--text-dim) text-[11px]">{topScorer.note}</div>
        </div>
        {scorerData && (
          <div className="text-right shrink-0">
            <div className="num text-[15px] text-(--accent)">{scorerData.prob}%</div>
            <div className="text-[10px] text-(--text-dim)">win prob</div>
          </div>
        )}
